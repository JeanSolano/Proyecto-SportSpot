...
> ⚠️ Usa `npm ci` (instala exactamente lo del `package-lock.json`). **No corras `npm audit fix --force`**: fuerza Vite 8 con cambios rompedores y el proyecto deja de arrancar. Los avisos de vulnerabilidades de npm son de dependencias de desarrollo y no afectan esta demo.
